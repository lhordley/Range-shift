...
nmrsdata[,c("Key","Code","Abundance","Life Stage","Date","Vice-county","Easting","Northing","_1k"):=NULL] 
# remove easting and northing because they are re-calculated below based on the 10km hectad/gridref
colnames(nmrsdata)[3] <- "Hectad"
...
library(rnrfa)
gridrefs <- as.vector(unique(nmrsdata[,3])) ## 2732 10km gridrefs
...
sqIM <- hectads[NORTHING < 505000 & NORTHING > 450000 &
                  EASTING > 200000 & EASTING < 250000]
ggplot(hectads, aes(EASTING, NORTHING))+
  geom_point()+coord_fixed()+
  geom_point(aes(EASTING, NORTHING), data = sqIM, color="red")

ggplot(hectads[!SQ10km %in% sqIM$SQ10km], aes(EASTING, NORTHING))+
  geom_point()+coord_fixed()

hectads <- hectads[!SQ10km %in% sqIM$SQ10km]

# Add easting and northing to records
bnmdata <- merge(bnmdata, hectads, by = "SQ10km") ## this removes Isle of Man records

## save NMRS cleaned data
saveRDS(nmrsdata, "../../Range shift/Data/NMRS/NMRS_hectad_cleaned.rds")
