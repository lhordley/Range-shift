...
#### date: October 2021
#### info: Upland species selection for range shift analysis
...
## These are given to Richard to determine which species to select
rm(list = ls())
...
########################################################################################################
## PLOT SPECIES TEMPERATURE RANGES (MEDIAN & 75TH PERCENTILE) for early and late TP
rm(list = ls())

nmrsdata_temp_early <- readRDS("../../Data/NMRS_hectad_temperature_early.rds") ## NMRS data for TP1 with temperature 
nmrsdata_temp_late <- readRDS("../../Data/NMRS_hectad_temperature_late.rds") ## NMRS data for TP2 with temperature 
migrant_hectads_early <- read.csv("../../Data/NMRS/migrant_hectads_exclusion_early.csv", header=TRUE) ## species x hectad combinations to remove due to immigrant populations
migrant_exclusion_late <- read.csv("../../Data/NMRS/migrant_hectads_exclusion_late.csv", header=TRUE)
...
nmrsdata_temp_early$TP <- "early"
nmrsdata_temp_late$TP <- "late"

## filter by upland/northern species only 
upland_species <- read.csv("../../Data/nmrs_temp_median_percentile_selection.csv", header=TRUE)
upland_species[is.na(upland_species)] <- 0 # replace NAs with 0 (0 = include the species as northern/upland)
upland_species_final <- upland_species[upland_species$exclude !=1, ] ## 73 species
upland <- upland_species_final[,1]
...
## plot all species boxplots with median and 25th/75th percentile for each time period
detach(package:plyr)
library(dplyr)
library(ggplot2)
summary_nmrs_temp <- nmrsdata_temp_early %>% 
...
sp_groups <- summary_nmrs_temp[,c(1,5)]
## use groups from above to put each species into groups to make plots
nmrsdata_temp_final <- merge(nmrsdata_temp_final, sp_groups, by="Common_Name", all=TRUE)
group <- unique(nmrsdata_temp_final$groups) ## 12 groups

nmrsdata_temp_final$TP <- with(nmrsdata_temp_final, relevel(TP, "early"))

ggplot(data = nmrsdata_temp_final[nmrsdata_temp_final$groups==1,], mapping = aes(x = reorder(Common_Name,-temperature, FUN=median), y = temperature, fill=TP)) +
  ...
  #scale_y_continuous(breaks = seq(3, 11, by = 1)) +
  theme_light()
ggplot(data = nmrsdata_temp_final[nmrsdata_temp_final$groups==2,], mapping = aes(x = reorder(Common_Name,-temperature, FUN=median), y = temperature, fill=TP)) +
  geom_boxplot(outlier.shape = NA, coef = 0) +
  #geom_hline(yintercept=9, linetype="dotted") +
  coord_flip() +
  labs(x="Common name", y="Temperature") +
  ...
  theme_light()
#
levels(as.factor(nmrsdata_temp_final$TP))
...
# ## plot distribution maps of: Rannoch Looper, Sword-grass, Great Brocade and Angle-striped Sallow
# rannoch_looper <- nmrsdata_temp[nmrsdata_temp$Common_Name=="Rannoch Looper",]
# sword_grass <- nmrsdata_temp[nmrsdata_temp$Common_Name=="Sword-grass",]
# great_brocade <- nmrsdata_temp[nmrsdata_temp$Common_Name=="Great Brocade",]
# angle_striped_sallow <- nmrsdata_temp[nmrsdata_temp$Common_Name=="Angle-striped Sallow",]
# 
# worldmap = map_data('world')
# 
# rannoch <- ggplot() + 
...
#                fill = 'gray90', color = 'black') + 
#   coord_fixed(ratio = 1.3, xlim = c(-10,3), ylim = c(50, 59)) + 
#   geom_point(data = rannoch_looper, 
#              aes(x = as.numeric(lon), 
#                  y = as.numeric(lat)), size=0.5) + 
#   ggtitle("Rannoch Looper") +
#   theme_void() +
#   theme(title = element_text(size = 12))
# sword <- ggplot() + 
#   geom_polygon(data = worldmap, 
...
#                fill = 'gray90', color = 'black') + 
#   coord_fixed(ratio = 1.3, xlim = c(-10,3), ylim = c(50, 59)) + 
#   geom_point(data = great_brocade, 
#              aes(x = as.numeric(lon), 
#                  y = as.numeric(lat)), size=0.5) + 
#   ggtitle("Great Brocade") +
#   theme_void() +
#   theme(title = element_text(size = 12))
# sallow <- ggplot() + 
#   geom_polygon(data = worldmap, 
...
#                fill = 'gray90', color = 'black') + 
#   coord_fixed(ratio = 1.3, xlim = c(-10,3), ylim = c(50, 59)) + 
#   geom_point(data = angle_striped_sallow, 
#              aes(x = as.numeric(lon), 
#                  y = as.numeric(lat)), size=0.5) + 
#   ggtitle("Angle-striped Sallow") +
#   theme_void() +
#   theme(title = element_text(size = 12))
# 
# library(gridExtra)
# library(grid)
# moths <- grid.arrange(rannoch, sword, brocade, sallow)
# ## save plot
# ggsave("../../Maps/moth_maps.png", plot=moths)
