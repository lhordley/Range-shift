...
recorded <- nmrsdata_rec[,c(1,2,11,12)]
...
## add in lat/lon info
lat_lon <- unique(nmrsdata_rec[,c(2,6,7)])
...
extirpated_hecs2 <- extirpated_hecs %>% group_by(Hectad, lat, lon) %>%
...
ggplot() +
  ...
  geom_point(data =extirpated_hecs2,
             aes(x = as.numeric(lon),
                 y = as.numeric(lat), colour=n_sp), size=2) +
...
## plot line graph
ggplot(elevation_prop, aes(x=elev_cat_int, y=freq, group=hec_cat)) +
